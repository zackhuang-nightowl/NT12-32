                                                   ...
                                                   nop_response_t *resp)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        nop_json_t *encoding = nop_json_obj();
        nop_json_t *resolution = nop_json_obj();
        nop_json_t *res_opts = nop_json_arr();
        char wh[32];
        int  have_opts = (nop_onvif_media2_get_venc_options(
                              onvif_session_dev(s), vencs[i].token, &opts) == 0);

        nop_json_add_str(e, "name", index_to_name(i));
                                                   ...
                                                   nop_response_t *resp)
{
    ...
    for (p = 0; p < np; p++) {
        ...
        if (!prof)
            continue;
        idx = name_to_index(nop_json_str(prof, "name", "main"));
        if (idx >= n)
            continue;
