 ...
 * NVR_ACT_AES_KEY / NVR_ACT_AES_IV（激活 AES256 盐）——都在 nvr_crypto.c 顶部，你填真实值。 */
...
/* ④ setDeviceActive 密码字段：AES256(P_act) 的十六进制串（IPC 解密后设为自身密码）。
 *   用 NVR_ACT_AES_KEY + NVR_ACT_AES_IV（盐）做 AES-256-CBC。返回 hex 长度，<0 失败。 */
...
/* 密钥是否已填（增强 KEY_X/KEY_Y 与激活 AES 密钥非空）。未填返回 0。 */
