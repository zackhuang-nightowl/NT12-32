...
rsdk_err_t rsdk_format(const char *path, const rsdk_format_opt_t *opt)
{
    ...
    uint32_t cmib  = pick_chunk_mib(sectors*RSDK_SEC, opt ? opt->chunk_mib : 0);
    double   mratio= (opt && opt->meta_ratio_pct > 0) ? opt->meta_ratio_pct : ((double)RSDK_CFG_META_RATIO_PCT10/10.0);

    uint64_t cs, cc, ds, bmS, ixS;
    compute_layout(sectors, cmib, slots, &cs, &cc, &ds, &bmS, &ixS);

    uint64_t meta_cc = (fmask & RSDK_FEAT_METADATA) ? (uint64_t)(cc * mratio / 100.0) : 0;
    if ((fmask & RSDK_FEAT_METADATA) && meta_cc == 0 && cc > 4) meta_cc = 1; /* 至少1个 meta chunk */
    uint64_t data_cc = cc - meta_cc;
    ...
    sb.total_sectors = sectors; sb.rsv_start_sec = 1; sb.data_start_sec = ds;
    sb.chunk_sectors = cs; sb.chunk_count = cc;
    sb.write_ptr_chunk = 0; sb.seq_epoch = 1;
    sb.feature_mask = fmask;
    sb.hdd_full = opt ? opt->hdd_full : (uint8_t)RSDK_CFG_HDD_FULL;
    sb.meta_start_chunk = data_cc; sb.meta_chunk_count = meta_cc;
    ...
    if (fmask & RSDK_FEAT_ENCRYPTION) {
        ...
        rnd_fill(dek, 32);
        rsdk_kdf_kek(opt && opt->sn ? opt->sn : RSDK_DEFAULT_SN, sb.kdf_salt, kek);
    ...
    st.bitmap_start_sec = BITMAP_SEC; st.bitmap_sectors = bmS;
    st.index_start_sec = BITMAP_SEC + bmS; st.index_sectors = ixS;
    st.index_slot_size = sizeof(rsdk_index_slot_t);
    st.index_slot_count = (uint32_t)(ixS * RSDK_SEC / sizeof(rsdk_index_slot_t));
    st.index_next = 0;
    st.meta_next_off = 0;
    st.meta_bytes = meta_cc * (cs * RSDK_SEC);      /* MetaRegion 字节大小 */
    ...
    for (uint64_t s = BITMAP_SEC; s < st.index_start_sec + ixS; s++)
...
rsdk_err_t rsdk_dev_open(const char *path, rsdk_dev_t **out)
{
    ...
    if (d->sb.enc_algo == 1) {  /* 装配加密: KEK←SN+salt, 解包 DEK */
        uint8_t kek[32], dek[32];
        rsdk_kdf_kek(RSDK_DEFAULT_SN, d->sb.kdf_salt, kek);
...
rsdk_err_t rsdk_dev_alloc_chunk(rsdk_dev_t *d, uint64_t *chunk, uint64_t *byte_off)
{
    ...
    if (d->sb.write_ptr_chunk >= d->data_chunks) {
        ...
        else { d->sb.write_ptr_chunk = 0; d->sb.seq_epoch++; }  /* overwrite: 回绕覆盖最旧 */
    }
    if (chunk) *chunk = c;
...
rsdk_err_t rsdk_dev_rekey(rsdk_dev_t *d)
{
    ...
    rsdk_kdf_kek(RSDK_DEFAULT_SN, new_salt, kek2);      /* 新盐 → 新 KEK */
