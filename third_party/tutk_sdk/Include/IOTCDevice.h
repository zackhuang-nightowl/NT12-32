...
typedef enum {
    IOTC_DEVICE_ADAPTION_MODE_NONE = 0, //!< No adaption
    IOTC_DEVICE_ADAPTION_MODE_CHOOSE_BEST_UDP_SERVER_OR_SWITCH_TO_TCP, //!< Choose best UDP server or switch to TCP
} IOTCDeviceAdaptionMode;
...
/**
 * \brief Device can let client detect in LAN
 *
 * \details Responds when IOTC Device receives a broadcast from IOTC Clients in LAN or not.
 *
 * \param enable_lan_find  [in] This is detectable device or not, default is enable.
 *
 * \attention (1) UID may exposure in LAN, when Device enable LAN find
 *
 */
P2PAPI_API void IOTC_Set_Device_Lan_Detectable(uint8_t is_detectable);
...
#if 0
/**
* \brief Used by a device to login to IOTC servers with Nebula
*
* \details ...
*
* \param device_ctx [in] Nebula context of device, it's from Nebula_Device_New()
*
* \return IOTC_ER_NoERROR if successfully login.
* \return Error code if return value < 0
*
* \see Nebula_Device_New()
**/
P2PAPI_API int32_t IOTC_Device_Login_By_Nebula(NebulaDeviceCtx* device_ctx);

/**
* \brief Used by a device to listen connections from clients with Nebula
*
* \details ...
*
* \param device_ctx [in] Nebula context of device, it's from Nebula_Device_New()
...
**/
P2PAPI_API int32_t IOTC_Device_Listen_By_Nebula(NebulaDeviceCtx *device_ctx, unsigned int timeout_msec);

#endif

/**
 * \brief Adapt the relay and login mode for the device under some special network environment.
 *
 * \details choose 
 * 
 * \param mode [in] Choose the mode of the network adaption
 *
 * \attention Can be called anywhere.
 *
 */
P2PAPI_API int32_t IOTC_Device_Setup_Network_Adaption(IOTCDeviceAdaptionMode mode);
