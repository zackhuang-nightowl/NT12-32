...
/**
 * \details The prototype of VSaaS configuration callback function used for AV server.
 *          The configuration function must be set in avEnableVSaaS.
 *          This function will be called while receiving VSaaS configuration in json format from av client.
 *
 * \param VSaaSconfig [in] The VSaaS configuration that user need to keep it in the storage.
 *
 */
typedef void(__stdcall *OnVSaaSConfigChangedCb)(const char *VSaaSconfig);

/**
 * \details The prototype of VSaaS contractinfo callback function used for AV server.
 *          The update contractinfo function must be set in avEnableVSaaS.
 *          This function will be called while receiving VSaaS pull stream message in json format from VSaaS server.
 *
 ...
 *
 */
AVAPI_API int32_t avEnableVSaaS(const char *udid, const char *VSaaSconfig, OnVSaaSConfigChangedCb config_callback, OnVSaaSUpdateContractInfoCb contract_callback);
 ...
 *          This function must be called before avInitialize().
 *
 * \param device_ctx [in] The context that create by Nebula module.
 * \param VSaaSconfig [in] The information that can connect to VSaas server.
 *                         You can get this from the registed OnVSaaSConfigChangedCb.
 *                         This param can be NULL if you never get information by OnVSaaSConfigChangedCb before.
 * \param config_callback [in] This callback function will be called while receiving VSaaSconfig from av client.
 ...
 *
 */
AVAPI_API int32_t avEnableVSaaSByNebula(NebulaDeviceCtx *device_ctx, const char *config, OnVSaaSConfigChangedCb config_callback, OnVSaaSUpdateContractInfoCb contract_callback);
#endif
 ...
 *          - #AV_ER_INVALID_ARG attr_obj is NULL or invalid.
 *
 *         Backend error code:
 *          - #AV_ER_VSAAS_INVALID_PARAMETER invalid parameter.
 *          - #AV_ER_VSAAS_INVALID_DEVICE invalid device or incorrect stoken.
 *          - #AV_ER_VSAAS_INVALID_BINDING invalid binding or no contract exists.
 *          - #AV_ER_VSAAS_INVALID_CONTRACT contract has been expired or cloud space is already fulled.
 *             or it has reached the usage limit.
 *          - #AV_ER_VSAAS_INVALID_START_TIME_RANGE invalid time range.
 *          - #AV_ER_VSAAS_INVALID_START_TIME_FORMAT invalid timestamp format. it should be in seconds.
 *          - #AV_ER_VSAAS_CONTRACT_NOT_FOUND contract not found.
 *
 * \see avEnableVSaaS()
 *
 */
AVAPI_API int32_t avServNotifyCloudRecordStream(TUTKJsonObject *attr_obj, uint32_t timeout_msec, uint32_t *abort_flag);
