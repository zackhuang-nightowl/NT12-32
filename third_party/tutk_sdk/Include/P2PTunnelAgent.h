 ...
 *			function is invoked.
 *
 * \param nMaxConnection [in] Specify max number of tunnel agents that can connect to tunnel servers
 * \param bEnableLanDirect [in] Transport data directly without TUTK forward mechanism when P2PTunnelAgent
 *			and P2PTunnelSever are on the same LAN.
 *
 * \return #TUNNEL_ER_NoERROR if initializing successfully
 * \return Error code if return value < 0
 *			- #TUNNEL_ER_ALREADY_INITIALIZED P2PTunnel module is already initialized
 ...
 *
 * \attention This function can't be called simultaneously from multiple threads.
 * \attention Lan direct isn't working with service mapping.
 */
 ...
 *          - #TUNNEL_ER_LOCAL_EXIT Force stop connecting process by other APIs.
 *          - #TUNNEL_ER_CREATE_DTLS_FAIL Create DTLS connection error.
 *
 */
...
/**
 * \brief Start connection for bacchus service
 *
 * \details This function used by a tunnel agent to start connection for a bacchus service
 *			provided by P2PTunnel module. The tunnel agent specifies the local port
 *			in local host through which a tunnel is created with the service id defined
 *			in the tunnel server.
 *
 * \param nSessionID [in] Which session ID of the P2PTunnel session to process the port mapping service
 * \param nLocalPort [in] The local port used to create a tunnel with the tunnel server
 * \param cszServiceID [in] The remote service id defined in the tunnel server to create a tunnel. Max length is MAX_SERVICE_ID_LEN(16).
 *
 * \return The service index if return value >= 0
 * \return Error code if return value < 0
 *			- #TUNNEL_ER_EXCEED_MAX_SERVICE Exceed the max number of service
 *			- #TUNNEL_ER_BIND_LOCAL_SERVICE Failed to bind local port since it is used by the other services
 *			- #TUNNEL_ER_LISTEN_LOCAL_SERVICE Failed to listen local port since it is used by the other services
 *			- #TUNNEL_ER_OS_RESOURCE_LACK Failed to malloc memory
 *			- #TUNNEL_ER_INVALID_ARG The session ID is not in valid range.
 *			- #TUNNEL_ER_REJECT Device reject this service.
 *			- #TUNNEL_ER_RETRY Call this API in multithread.
 */
 ...
 * \brief Stop service
 *
 * \details This function used by a tunnel agent to stop service
 *			on a given port mapping index which is started by P2PTunnelAgent_ServiceMapping()
 *
 * \param nServiceMappingIdx [in] The port mapping index started in P2PTunnelAgent_ServiceMapping()
 *
 * \attention If not call P2PTunnelAgent_ServiceMapping local port can't be used until OS release it.
 */
 ...
 * \param indexNum [in] The number of the port mapping index in indexArray
 *
 * \return 0 if success
 * \return Error code if return value < 0
