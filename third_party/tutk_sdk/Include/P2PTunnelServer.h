...
/**
 * \details The prototype of Tunnel server to verify Service ID.
 *
 * \param cszServiceID [in] Service ID that Tunnel agent want to access. Max length is MAX_SERVICE_ID_LEN(16).
 * \param pArg [in] Tunnel server pass user data
 *
 * \return Service port number if service ID passes.
 * \return 0 to reject access
 */
 ...
 *			function is invoked.
 *
 * \param nMaxAgentConnection [in] Specify max number of tunnel agents that can connect to this tunnel server
...
/**
 * \brief Register Callback function for verify service id.
 *
 * \details When Tunnel server recevice request for access service at specific service id from remote, SDK will invoke callback function.
 *
 * \param pfxTunnelServiceVerifyFn [in]  The function pointer to verify service id function.
 * \param pArg [in] User can give data pointer to pass to pfxTunnelServiceVerifyFn when this callback function is triggered. Can be NULL.
 *
 * \return #TUNNEL_ER_NoERROR if register successfully
 * \return Error code if return value < 0
 *			- #TUNNEL_ER_NOT_INITIALIZED P2PModule has not been initialized in that tunnel server
...
/**
 * \brief Used to gracefully stop P2PTunnelServer_Listen process
 *
 *
 * \param SID [in] Base on which IOTC session you want to stop P2PTunnel from listening on 
 * \param CHID [in] Base on which IOTC channel id you want to stop P2PTunnel from listening on 
 *
 * \return #TUNNEL_ER_NoERROR if deinitialize successfully
 * \return Error code if return value < 0
 *			- #TUNNEL_ER_INVALID_ARG if the channel id is out of range
