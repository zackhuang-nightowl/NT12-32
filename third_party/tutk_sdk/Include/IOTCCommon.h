...
typedef enum {
    IOTC_DATA_TRANSMIT_ADAPTATION_MODE = 0,
    IOTC_DATA_TRANSMIT_INTEGRITY_MODE = 1,    //!< Device SDK version >= 4.3.7.x
    IOTC_DATA_TRANSMIT_COMPATIBILITY_MODE = 2 //!< Device SDK version <= 4.3.6.x or Device using compatibility library
} IOTCDataTransmitMode;
...
typedef struct st_SInfoEx {
    ...
    uint8_t isNebula; //!< 0: Session not created by nebula, 1: Session created by nebula
    uint8_t isUseAuthkey; //!< 0: Session connected without IOTC authkey, 1: Session connected with IOTC authkey
    IOTCDataTransmitMode DataTransmitMode; //!< 1:integrity mode, 2:compatibility mode
} st_SInfoEx_t;
...
typedef struct IOTCConnectInput {
    ...
    IOTCDataTransmitMode data_transmit_mode; //!< 0:Adaptaion mode, 1:Integrity mode, 2:Compatibility mode
} IOTCConnectInput;
 ...
 *          After calling this API, the issues of socket failures when an app moves from the background 
 *          to the foreground and the blocking of UDP ports in some network environments after a period 
...
/**
 * \brief Set RSA Public Key for Master Server
 * 
 * \details This API is useful for when there is a need to set a specific key for master server.
 *          If not set, the "default" key will be used.
 * 
 * \param nMasterKey [in] The RSA Public key that should be used
 * 
 * \return Returns IOTC_ER_NoError if the set was successful
 * \return or an error code if value < 0 
 *          - #IOTC_ER_INVALID_ARG if the input is not valid
 * 
*/
P2PAPI_API int32_t IOTC_Set_Master_Pub_Key(const char *nMasterKey);
