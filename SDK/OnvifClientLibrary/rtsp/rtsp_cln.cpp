...
#ifdef MEDIA_DEVICE     // byte-push backchannel needs no mic-capture headers
#if __WINDOWS_OS__
...
#ifdef BACKCHANNEL
    m_rua.bc_audio_device = -1; // byte-push default (no mic capture device)
...
#ifdef BACKCHANNEL
        // Only start mic capture when a real audio device is selected.
        // byte-push mode (bc_audio_device < 0): skip capture; bc_rtp_info
        // was already initialized at SETUP of AV_BACK_CH.
        if (p_rua->backchannel && m_rua.bc_audio_device >= 0)
        {
...
BOOL CRtspClient::send_backchannel(uint8 * data, int len, int nbsamples)
{
    if (!m_rua.backchannel || !m_rua.send_bc_data) return FALSE;
    rtsp_bc_cb(data, len, nbsamples, &m_rua);   // builds RTP (pt/seq/ts/ssrc), SRTP-protects, sends over TCP/UDP/WS
    return TRUE;
}
...
{
#ifndef MEDIA_DEVICE
    // Byte-push mode (no mic-capture engine): the app supplies talk audio via
    // CRtspClient::send_backchannel(). Only reached if bc_audio_device >= 0,
    // which byte-push mode never sets (default -1).
    (void)p_rua;
    return FALSE;
#else
    int count;
...
#if defined(BACKCHANNEL) && defined(MEDIA_DEVICE)
