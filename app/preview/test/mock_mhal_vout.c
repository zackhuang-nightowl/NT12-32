...
int mhal_vout_bind(int win,int chn){ /* 宫格绑定：记为 rect 未知，仅记 chn + win */
    mhalmock_bind_win[mhalmock_nbind] = win;
    mhalmock_rects[mhalmock_nbind++] = (mock_rect_t){chn,-1,-1,-1,-1}; return 0; }
int mhal_vout_bind_rect(int chn,int x,int y,int w,int h){
    mhalmock_bind_win[mhalmock_nbind] = -1;
