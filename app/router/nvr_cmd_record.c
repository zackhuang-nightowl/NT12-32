...
/* 读 DB；无行返回错误(GET 不得 C 层填默认,出厂 seed 保证有行)。 */
static int record_load(const nvr_cmd_ctx_t *c, int chn, nvr_record_cfg_t *r)
{
    if (!c || !c->settings) return -1;
    return nvr_settings_record_get(c->settings, chn, r);
}
static int rec_sched_load(const nvr_cmd_ctx_t *c, int chn, nvr_rec_schedule_t *r)
{
    if (!c || !c->settings) return -1;
    return nvr_settings_rec_sched_get(c->settings, chn, r);
}
...
char *cmd_X_NightOwl_setChannelRecordingTriggers(cJSON *a, const nvr_cmd_ctx_t *c)
{
    ...
    int chn0 = ch1 - 1;
    nvr_record_cfg_t r;
    if (record_load(c, chn0, &r) != 0) return nvr_resp_err("no_config");
...
char *cmd_X_NightOwl_setChannelContinuousScheduleRecordingSwitch(cJSON *a, const nvr_cmd_ctx_t *c)
{
    ...
    nvr_rec_schedule_t r;
    if (rec_sched_load(c, chn0, &r) != 0) return nvr_resp_err("no_config");
    r.sched_on = on;
...
char *cmd_GUI_getChannelEventRecordingSchedule(cJSON *a, const nvr_cmd_ctx_t *c)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        cJSON_AddStringToObject(rule, "id", rows[i].rule_id);
        cJSON *wd = cJSON_CreateArray();
        weekdays_csv_to_arr(rows[i].weekdays, wd);
        cJSON_AddItemToObject(rule, "weekdays", wd);
        cJSON_AddStringToObject(rule, "startTime", rows[i].start_hms);
        cJSON_AddStringToObject(rule, "endTime", rows[i].end_hms);
...
char *cmd_getChannelRecordingTime(cJSON *a, const nvr_cmd_ctx_t *c)
{
    ...
    int chn0 = ch1 - 1;
    if (!c || !c->settings) return nvr_resp_err("invalid_param");
    int v = nvr_settings_record_post_s_get(c->settings, chn0);
    if (v < 0) return nvr_resp_err("no_config");
    cJSON *o = cJSON_CreateObject();
